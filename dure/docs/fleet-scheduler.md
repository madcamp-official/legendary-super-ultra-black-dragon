...
> 상태: **`fleet-candidate-v3` 불변 추천, 원자적 수락·예약과 전용 Fleet runtime 구현**. 네 모델의 ACTIVE 자동 배치 프로필과 현재 유효한 exact qualification 증적을 GPU 풀에 투영해 서로 겹치지 않는 여러 배포 단위를 결정론적으로 조합합니다. 추천과 수락은 호스트를 바꾸지 않으며, 운영자가 전용 준비와 적용 명령을 각각 명시한 뒤에만 모델·이미지 준비와 컨테이너 적용·검증을 수행합니다.
...
완전한 최신 probe가 exact rank의 `STAGE` identity를 `PRESENT`로 확인한 경우에는
준비기가 해당 캐시를 다시 전체 해시하므로 재다운로드용 `2 × rank bytes + 64 MiB`의
여유 공간을 후보 단계에서 다시 요구하지 않습니다. exact 관찰이 없거나 digest·rank가
다르면 기존 보수적 다운로드 여유 공간 검사를 그대로 적용합니다. 컨테이너가 중지되어
호스트 자원을 소유하지 않는 `PLANNED` 관측 상태도 후보 점유로 보지 않지만, Fleet
수락 뒤 예약은 기존 배포의 후속 START 작업과 충돌하지 않도록 계속 강제됩니다.
