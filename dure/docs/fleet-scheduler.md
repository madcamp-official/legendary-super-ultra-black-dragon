...
0.3.33의 `tests/test_fleet_scale_acceptance.py`는 실제 하드웨어를 대신하는 성능 시험이 아니라 합성 인벤토리와 검증 후보를 사용한 8개의 결정론적 단위·회귀 검사입니다. 저장·runtime 계층의 기존 회귀 검사는 SQLite를 사용합니다. 다음 경계를 함께 검증합니다.
