...
> 상태: **내부 읽기 전용 평가 구현**. 네 모델의 ACTIVE 자동 배치 프로필과 현재 유효한 exact qualification 증적을 GPU 풀에 투영하고, 서로 겹치지 않는 여러 배포 단위를 결정론적으로 조합합니다. 영속 Fleet 추천, 관리자 API·CLI, 원자적 수락·예약과 실제 적용은 후속 단계입니다.
...
현재 노드 프로필 자체에는 운영자가 지정한 네트워크 영역 값이 없습니다. 평가 호출자가 서버가 신뢰하는 명시적 `node UUID → network zone` 매핑을 제공하면, 모든 rank가 같은 영역인 후보의 비용은 0이고 여러 영역을 가로지르는 후보는 서로 다른 영역 수에 따라 비용을 받습니다. 매핑이 없거나 후보의 일부 노드에만 있으면 추측하지 않고 중립값을 사용합니다. 어느 경우에도 exact NCCL 증적은 필수입니다. 영속 추천과 수락·예약·적용 경계는 후속 Fleet API 문서에서 이어서 정의합니다.
