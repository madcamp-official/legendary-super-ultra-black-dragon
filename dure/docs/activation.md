...
- 중앙 서버와 대상 Agent가 모두 0.3.25 이상이어야 합니다.
- 대상 노드는 Docker와 NVIDIA runtime이 준비되고 승인·온라인이어야 합니다.
- runtime 이미지는 `repository@sha256:<digest>` 형식이며 `dure-benchmark` 진입점을
  제공해야 합니다.
- 모델은 불변 commit revision과 정규 manifest를 가져야 합니다.
- 각 대상 Agent의 root 전용 `/etc/dure/agent.json`에는 기존 credential을 유지한 채
  신뢰 HTTPS `artifact_origin`이 설정돼야 합니다. 설정 파일 전체를 출력하거나
  공유하지 않습니다.
