...
- [모델 아티팩트 매니페스트와 배포 계약](docs/artifact-distribution.md) — `FULL_SNAPSHOT` 구현, 분산 `STAGE`는 계획됨
...
- 중앙 준비는 모델 전체를 각 대상 노드에 두는 `FULL_SNAPSHOT`만 지원합니다. rank별 파일을 생성·배포하는 `STAGE`, 노드 간 P2P 전송, 실제 다중 노드 NCCL 실행과 모델 파일 자체의 분할 배포는 아직 지원하지 않습니다.
