...
- 검증된 `STAGE` variant를 명시적으로 선택해 pipeline rank별 매니페스트를 각 노드의 분리된 캐시에 준비하고 vLLM `sharded_state`로 읽는 다중 노드 경로
...
`VLLM_RAY_PP_V1`은 정확히 vLLM 0.9.0 V0 executor와 Ray를 사용합니다. GCS는 `6379`, Ray worker 범위는 `20000-21000`으로 고정하고 vLLM API는 head의 `127.0.0.1:8000`에만 바인딩합니다. 각 컨테이너는 배포·세대·노드 외에 backend, pipeline rank, runtime rank, component 레이블까지 정확히 일치해야 시작·검증·중지·재시작 대상이 됩니다. `FULL_SNAPSHOT`과 명시적으로 선택한 검증 `STAGE`를 지원하며 둘 사이의 묵시적 fallback은 없습니다.
...
- 중앙 준비는 생략 시 기존 `FULL_SNAPSHOT`을 유지하고 `--stage-variant sha256:...`를 명시한 경우에만 정확한 `VALIDATED` variant의 rank별 매니페스트를 준비합니다. Agent는 계약 전체에서 계산한 노드별 캐시 경로에 marker-last로 활성화하고 vLLM에는 모두 `/models/model`로 읽기 전용 mount합니다. 추천기가 variant를 자동 선택하는 기능과 중앙 `READY`·`STALE`·`MISSING`·`CORRUPT`·`QUARANTINED` 캐시 투영은 아직 구현되지 않았습니다. `STAGE` 실패 시 `FULL_SNAPSHOT`으로 자동 fallback하거나 실행 중인 이전 배포를 자동 중지·교체·롤백하지 않습니다.
- 일반 probe·heartbeat는 수 GiB stage 파일을 매번 재해시하지 않습니다. 관측한 `STAGE` identity는 인벤토리에 `complete=false`로 투영되며 `READY` 증거가 아닙니다. 준비 완료 검증과 Docker 시작 직전 runtime 재해시가 현재의 권위 있는 무결성 게이트입니다.
...
# 검증된 STAGE variant를 사용하려면 preview와 apply 모두 같은 digest를 명시합니다.
dure admin deployment prepare <deployment-id> \
