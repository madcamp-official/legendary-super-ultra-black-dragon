...
- 현재 `--model auto`는 버전이 있는 Qwen2.5 AWQ 정적 카탈로그와 결정론적 선택기를 사용합니다. 중앙 모델 레지스트리, 저장 인벤토리 기반 읽기 전용 추천, 벤치마크 증적 기반 `ACTIVE` 승격 게이트와 안전한 단일 GPU 실행 경로를 제공합니다. 추천 승인과 배포 세대 전환은 [모델 선택 정책](docs/model-selection.md)의 후속 범위입니다. 선택기의 기본 정책과 중앙 추천은 네트워크·NCCL 증적이 없는 다중 노드 후보를 거부하지만, 로컬 `dure plan --model auto`는 기존 3×24GB 계획 호환성을 위해 이 검사만 예외로 두고 계획에 검증 경고를 남깁니다.
- 중앙 제어면은 승인된 단일 노드에서 고정된 네 가지 작업 부하를 `BENCHMARK` 작업으로 실행하고 결과를 증적으로 수집할 수 있습니다. 자동 실행은 `/var/lib/dure/models`에 펼쳐지고 `.dure-model.json`으로 고정 식별자를 결합한 모델과 로컬 다이제스트 이미지를 요구하며, 다운로드·이미지 pull·Hugging Face hub snapshot, 다중 노드 네트워크·NCCL 시험, 전체 동시성 매트릭스와 24시간 복구 검증은 지원하지 않습니다. 저장된 네트워크·NCCL 증적을 중앙 추천의 노드 자격에 연결하는 기능도 후속 범위입니다.
