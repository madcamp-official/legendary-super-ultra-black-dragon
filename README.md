...
구조화된 네트워크·NCCL 증적 저장과 승격 판정은 구현됐지만, 추천기가 이를 노드 조합 적격성 입력으로 조회하는 연계와 자동 다중 노드 시험은 아직 구현되지 않았습니다. 따라서 중앙 다중 노드 후보는 추천에서 실패 안전 방식으로 거부됩니다. 현재 세대 검증과 롤백도 전체 작업 부하 매트릭스, 네트워크·NCCL 시험 또는 24시간 복구 검증을 대신하지 않습니다.
