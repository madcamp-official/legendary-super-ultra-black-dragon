...
추천 생성과 수락은 `PROBE`나 배포 작업을 만들지 않고, 모델 다운로드, 이미지 내려받기, Docker 실행, 기존 컨테이너 중지를 유발하지 않습니다. 실제 호스트 변경에는 별도의 `dure admin apply`가 필요합니다. 구조화된 네트워크·NCCL 증적을 중앙 추천의 노드 조합 자격에 연결하는 기능은 아직 없어 다중 노드 후보는 실패 안전 방식으로 거부됩니다.
