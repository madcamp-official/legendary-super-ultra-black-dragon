...
GPU 추가에 따른 정책 기반 모델 추천, 불변 스냅샷 저장과 명시적 수락을 통한 적용 전 배포 세대 생성은 구현되었습니다. 기존 명시적 apply·verify 작업은 계속 사용할 수 있지만, 세대별 영속 작업 상태·검증 완료 기록과 명시적 롤백은 후속 구현 범위이며 다음 경계를 유지합니다.
...
불변 배포 세대 생성 (현재 구현, 작업 없음)
        ↓
명시적 적용·검증 (기존 작업) → 세대별 기록·롤백 (후속 구현)
```
...
추천기는 모델의 이름·크기만 비교하지 않고 GPU VRAM, 드라이버·연산 능력·런타임 지원 아키텍처, 디스크, 모델 아티팩트, 런타임 이미지, 컨텍스트·동시성, 네트워크/NCCL 조건을 평가합니다. 구조화된 네트워크·NCCL 증적 저장은 구현되었지만 추천기가 이를 노드 조합의 적격성 입력으로 조회하는 연계는 아직 없어 모든 중앙 다중 노드 후보를 실패 안전 방식으로 거부합니다. 현재 계획 전송 형식으로 안전하게 표현할 수 없는 TP 계열 배치도 수락 단계에서 거부합니다. 모델 레지스트리, 아티팩트 검증, 세대별 단계적 전환의 상세 정책은 [모델 선택 정책](model-selection.md)과 [벤치마크 문서](benchmarking.md)를 따릅니다.
