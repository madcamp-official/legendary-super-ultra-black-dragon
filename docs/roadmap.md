...
기준일: 2026-07-21
현재 누적 개발 브랜치: `version/0.3.15` (콘텐츠 주소 기반 로컬 모델 캐시 준비기, 공식 병합 전 Draft)
...
- 다음 PR: 명시적 중앙 준비 operation·API·CLI, 폐쇄형 `PREPARE_MODEL`·`PREPARE_IMAGE`, digest-pinned OCI pull과 노드별 READY 증적
- 그 이후: rank별 `STAGE` 아티팩트 배포와 결정론적 다중 노드 런타임 결합

현재 누적 `version/0.3.15` 범위는 추천과 수락의 무변경 경계를 유지하면서 apply·verify operation, 노드별 상태와 task 시도 번호를 기록합니다. 계획의 전체 배정 노드가 모두 검증에 성공하고 모든 Agent가 0.3.12 이상일 때만 `verified_at`을 기록합니다. 최신 세대는 직접 연결된 상태가 `VERIFIED`이고 `verified_at`을 보유한 직전 세대로만 롤백할 수 있으며, 기본 요청은 준비만 하고 `apply=true`에서만 호스트 변경을 시작합니다. 다중 노드 추천은 정확한 노드 조합에 결합된 최신 네트워크·NCCL 증적을 요구합니다.

아티팩트 매니페스트는 중앙 DB에 정규 파일·청크 관계를 저장하고, 새 노드 라이브러리는 노드 로컬 신뢰 origin에서 해당 청크를 받아 `FULL_SNAPSHOT` 캐시로 materialize할 수 있습니다. 중단 다운로드와 조립을 재개하고, CAS·파일·전체 트리·양자화 식별자를 다시 검사한 뒤 marker-last와 no-replace rename으로 활성화합니다. 그러나 중앙 준비 API·CLI, `PREPARE_MODEL`·`PREPARE_IMAGE` Agent 작업, READY 증적과 배포 소비 게이트는 아직 없습니다. 따라서 추천·수락·GPU 추가는 자동 준비를 시작하지 않으며 이 누적 브랜치는 공식 `main`에 병합되기 전까지 Draft 개발 상태입니다.
