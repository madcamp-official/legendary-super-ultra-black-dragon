...
기준일: 2026-07-20
현재 개발 브랜치: `version/0.3.11` (추천 수락과 적용 전 배포 세대 생성)
...
- `version/0.3.12` 이후: 세대별 검증 상태와 명시적 롤백

현재 `version/0.3.11` 범위는 추천과 정규화 인벤토리를 불변 스냅샷으로 저장하고, 운영자가 명시적으로 수락할 때 현재 상태를 다시 평가해 일치하는 적용 전 배포 세대만 만듭니다. 이전 세대를 지정하면 같은 계보의 최신 세대에만 연결합니다. 추천과 수락은 작업, 다운로드, 이미지 내려받기, Docker 실행이나 기존 서비스 중지를 만들지 않습니다. 세대별 apply·verify 상태와 rollback은 다음 버전 범위입니다.
...
현재 상태: 결정론적 선택기, 중앙 모델 레지스트리, 구조화된 증적과 승격 게이트, 폐쇄형 단일 GPU 벤치마크, 추천 스냅샷과 명시적 수락은 구현되었습니다. 네트워크·NCCL 자동 증적, 전체 작업 부하 매트릭스, 세대별 적용·검증·롤백이 남아 있어 다중 노드 완료 기준은 아직 충족하지 않았습니다.
