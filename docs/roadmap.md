...
기준일: 2026-07-21
현재 누적 개발 브랜치: `version/0.3.16` (명시적 중앙 모델·이미지 준비 작업, 공식 병합 전 Draft)
...
- 다음 PR: 검증된 vLLM stage 아티팩트 variant 레지스트리와 신뢰된 전용 빌더
- 그 이후: 결정론적 다중 노드 rank 결합, rank별 `STAGE` 준비·로더 통합과 probe 기반 캐시 수명주기

현재 누적 `version/0.3.16` 범위는 추천과 수락의 무변경 경계를 유지하면서 apply·verify operation, 노드별 상태와 task 시도 번호를 기록합니다. 계획의 전체 배정 노드가 모두 검증에 성공하고 모든 Agent가 0.3.12 이상일 때만 `verified_at`을 기록합니다. 최신 세대는 직접 연결된 상태가 `VERIFIED`이고 `verified_at`을 보유한 직전 세대로만 롤백할 수 있으며, 기본 요청은 준비만 하고 `apply=true`에서만 호스트 변경을 시작합니다. 다중 노드 추천은 정확한 노드 조합에 결합된 최신 네트워크·NCCL 증적을 요구합니다.

아티팩트 매니페스트는 중앙 DB에 정규 파일·청크 관계를 저장하고, 노드 라이브러리는 노드 로컬 신뢰 origin에서 해당 청크를 받아 `FULL_SNAPSHOT` 캐시로 materialize합니다. 중단 다운로드와 조립을 재개하고, CAS·파일·전체 트리·양자화 식별자를 다시 검사한 뒤 marker-last와 no-replace rename으로 활성화합니다. 중앙 준비 API·CLI와 `PREPARE_MODEL`·`PREPARE_IMAGE` Agent 작업은 preview와 명시적 적용을 분리하며, 실패 노드의 현재 단계만 재시도합니다. 추천 세대의 일반 apply는 전체 노드의 정확한 준비 성공 증적이 없으면 거부합니다. 다만 probe와 조정되는 독립 `READY` 캐시 수명주기, rank별 `STAGE` 파일과 분산 로더는 아직 없으며, 추천·수락·GPU 추가만으로 자동 준비를 시작하지 않습니다. 이 누적 브랜치는 공식 `main`에 병합되기 전까지 Draft 개발 상태입니다.
