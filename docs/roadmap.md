...
현재 `version/0.3.12` 범위는 추천과 수락의 무변경 경계를 유지하면서 apply·verify operation, 노드별 상태와 task 시도 번호를 기록합니다. 계획의 전체 배정 노드가 모두 검증에 성공하고 모든 Agent가 0.3.12 이상일 때만 `verified_at`을 기록합니다. 최신 세대는 직접 연결된 상태가 `VERIFIED`이고 `verified_at`을 보유한 직전 세대로만 롤백할 수 있으며, 기본 요청은 준비만 하고 `apply=true`에서만 호스트 변경을 시작합니다.
