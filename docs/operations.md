...
정책 기반 `recommend`, 벤치마크 승격 게이트와 단일 GPU 폐쇄형 벤치마크는 구현되어 있으며 추천 승인과 세대별 단계적 전환은 아직 구현되지 않았습니다. 추천은 데이터베이스에 저장된 노드 프로필과 `ACTIVE` 모델 릴리스만 조회합니다.
