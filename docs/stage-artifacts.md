...
> 상태: **생성·등록·검증 계층 구현, 노드 배포 소비는 다음 PR로 계획됨**. Dure 0.3.17은 제한된 vLLM 계약으로 rank별 `STAGE` 아티팩트를 만드는 신뢰된 오프라인 빌더와, 그 결과를 `DRAFT`·`VALIDATED`·`REVOKED`로 관리하는 중앙 레지스트리를 제공합니다. 0.3.18의 `VLLM_RAY_PP_V1`은 결정론적 다중 노드 rank 결합을 실행하지만 여전히 각 노드의 `FULL_SNAPSHOT`만 읽습니다. Agent가 `STAGE`를 내려받아 활성화하거나 추천·준비·배포가 stage variant를 선택하는 기능은 아직 제공하지 않습니다.
...
builder와 중앙 레지스트리는 PP 1~64의 구조를 표현하지만, stage builder용 실제 GPU acceptance harness는 `PP=1`만 native export·load·추론까지 검증합니다. PP>1 variant는 구조 검사와 `DRAFT` 등록까지 가능하더라도, 모든 rank의 stage 경로를 실제로 load하고 attest하는 별도의 신뢰된 검증이 없으면 `VALIDATED`로 운영하지 않습니다. 0.3.18은 `PP=2/3`의 `FULL_SNAPSHOT` Ray 실행과 소스 고정 rank 계약을 추가했을 뿐 PP>1 `STAGE` load를 검증하지 않습니다. stage-local `sharded_state` 로더와 rank별 준비 증적은 다음 PR 범위입니다.

## 현재 다중 노드 실행과의 관계

`VLLM_RAY_PP_V1`과 stage variant는 서로 다른 계약입니다.

| 구분 | 0.3.18 현재 구현 | 다음 PR 계획 |
|---|---|---|
| 모델 캐시 | 모든 노드의 검증된 `FULL_SNAPSHOT` | 각 노드 rank에 고정된 `STAGE` |
| 실행 토폴로지 | vLLM 0.9.0 V0 Ray, `TP=1`, `PP=2/3` | 같은 엄격한 backend에서 stage-local `sharded_state` 소비 |
| rank 결합 | 서버 UUID, head rank 0, worker RFC1918 IPv4 문자열 정렬 | 같은 결합에 stage manifest·exact path 증적 추가 |
| 준비 실패 | 새 실행 단계 차단, 명시적 재시도·롤백 | 실패 rank만 재준비하되 full/stage 자동 fallback 금지 |

현재 `pipeline-rank-contract`는 고정된 vLLM 0.9.0 소스 규칙과 Ray 노드·actor topology를 대조하는 간접 증적입니다. Ray가 vLLM 내부 pipeline rank 값을 직접 보고했다는 뜻이 아니며, stage 파일이 해당 rank에서 실제 load됐다는 증거도 아닙니다. `scripts/acceptance-vllm-ray-pp.py`의 2·3노드 GPU 검사는 현재 `FULL_SNAPSHOT` 분산 load를 대상으로 하므로 stage variant의 `GPU_EXPORT_LOAD/PASSED`를 대신하지 않습니다.
...
다음 PR에서는 현재 레지스트리의 `VALIDATED` variant를 `VLLM_RAY_PP_V1` 배포 계층이 소비하도록 별도의 안전 검사를 추가해야 합니다.
...
그 다음 PR에서는 probe 결과와 중앙 캐시 투영을 조정해 `READY`·`STALE`·`MISSING`·`CORRUPT`·`QUARANTINED` 수명주기와 명시적 격리 절차를 추가할 계획입니다. 현재 문서나 runtime의 실패를 이 상태가 이미 구현된 것으로 해석해서는 안 됩니다.
