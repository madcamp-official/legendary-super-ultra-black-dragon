...
## 계획된 모델 레지스트리와 추천기의 경계

정책 기반 추천은 호스트 변경 권한이 아닙니다. 추천은 승인된 최신 인벤토리와 `ACTIVE` 모델 릴리스만 평가하며, 배포 구성 생성과 적용은 별도 운영자 행동입니다.

- 아티팩트는 변경 불가능한 리비전과 매니페스트 다이제스트를 가져야 합니다.
- 런타임 이미지는 정확한 OCI 다이제스트로 고정합니다.
- 라이선스와 사용 조건은 릴리스 승격 전에 검토합니다.
- 레지스트리에는 임의 셸, Docker 인자 목록, 호스트 경로, 마운트, 환경 변수를 저장하지 않습니다.
