...
- 구조화된 네트워크·NCCL 증적 저장과 승격 판정은 구현되었지만 추천기가 이를 노드 조합 자격에 연결하는 기능은 아직 없습니다. 따라서 중앙 추천은 다중 노드 후보를 실패 안전 방식으로 거부하며, 로컬 `dure plan --model auto`의 기존 3×24GB 호환 예외만 계획에 사전 검증 경고를 남기고 이 검사를 건너뜁니다.
